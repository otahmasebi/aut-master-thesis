...
import torch
import torchaudio
from data_module import CommomVoiceDataModule
...
def _extract_labels(sp_model, samples: List):
    targets = [sp_model.encode(sample[2]['sentence'].lower()) for sample in samples]
...
def get_data_module(dataset_path, global_stats_path, sp_model_path):
