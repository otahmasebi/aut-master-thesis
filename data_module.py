...
def get_sample_lengths(commom_voice_dataset):
    fileid_to_target_length = {}

    def _target_length(item_file):
        file_id = item_file[0]
        transcript = item_file[2]
        if file_id not in fileid_to_target_length:
            fileid_to_target_length[file_id] = len(transcript)
            # speaker_id, chapter_id, _ = fileid.split("-")

            # file_text = speaker_id + "-" + chapter_id + commom_voice_dataset._ext_txt
            # file_text = os.path.join(commom_voice_dataset._path, speaker_id, chapter_id, file_text)
        ...
        return fileid_to_target_length[file_id]

    return [_target_length(fitem_file) for fitem_file in commom_voice_dataset._walker]
...
class CommomVoiceDataModule(LightningDataModule):
    common_voice_cls = torchaudio.datasets.COMMONVOICE
    ...
    def train_dataloader(self):
        datasets = [
            self.common_voice_cls(self.dataset_path, tsv="validated.tsv"),
        ]
        ...
        # dataset = CustomBucketDataset(dataset, None, self.max_tokens, self.train_num_buckets, batch_size=self.batch_size)
    ...
    def val_dataloader(self):
        datasets = [
            self.common_voice_cls(self.dataset_path, tsv="dev.tsv"),
        ...
        datalot = TransformDataset(dataset, self.val_transform)
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=None, num_workers=self.num_workers)
        return dataloader

    def test_dataloader(self):
        dataset = self.common_voice_cls(self.dataset_path, url="test.tsv")
